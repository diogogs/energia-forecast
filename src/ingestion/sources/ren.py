...
# Valid slot counts for a full day: 92 (23h spring-forward), 96 (24h), 100 (25h fall-back).
_VALID_SLOT_COUNTS = frozenset({92, 96, 100})
...
def parse_production_breakdown(payload: JsonObj, day: dt.date) -> list[RenObservation]:
    ...
    if n_slots not in _VALID_SLOT_COUNTS:
        valid = sorted(_VALID_SLOT_COUNTS)
        raise RenParseError(f"unexpected slot count {n_slots} (not a valid 15-min day: {valid})")
...
def _post(client: httpx.Client, url: str) -> httpx.Response:
    return client.post(
        url, content="{}", headers={"Content-Type": "application/json"}, timeout=40.0
    )
