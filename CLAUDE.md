...
- [ ] **⚠️ Migrar triggers para cron-job.org (ADR-013)** — medições 07-09/10: o GitHub atrasa TODOS os eventos schedule ~3h20 (retry tickets inúteis) e descarta ~90% dos de alta frequência (keepalive disparou 5×/dia → Render adormece). Emissão de 07-10 às 10:26 UTC falhou até o leilão. Sistema impecável, scheduler inviável. Ação do utilizador: conta cron-job.org + fine-grained PAT (Actions RW, só este repo) → 3 jobs (keepalive GET /health 10min; dispatch ingest 06:30; dispatch predict 07:05). Schedules do GH ficam como fallback. Só depois disto começa a contagem dos 3 dias de autonomia pontual.
