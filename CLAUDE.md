...
**Estado global — ambos os targets modelados e a baterem baselines; Fase-1 live, Fase-2 pronta.** As 4 fontes na camada `raw` (~1.85M linhas: OMIE 84k, REN 1.06M, Energy-Charts 353k, Open-Meteo 358k) → fundação temporal anti-leakage (`AsOfRepo`, publicação modelada, ADR-011) → `build_features` (calendário Lisboa, lags legais, meteo). **Fase 1 (consumo):** LightGBM `regression_l1` MAE 166 MW / **MAPE 2.77%** vs sazonal 5.95% → **ACEITE**; **live** via 3 crons GitHub Actions (`ingest` 06:30, `predict` 07:05, `backtest` semanal), previsões em `pred.*` (insert-only). **Fase 2 (preço):** tripleto quantílico P10/P50/P90, **P50 MAE 13.24 €/MWh** vs persistência 15.98 → **ACEITE**, cobertura P10-P90 74.9%; modelo pronto, **falta ligar o emit diário**. Tracking MLflow (DagsHub-ready, fallback local). Migrações **0001-0006**, ADRs **001-011**, **108 testes verdes** (marker `leakage` = gate), CI verde, custo zero (Neon 430/512 MB). **A seguir:** emit diário de preço → calibração conformal do intervalo → dashboard/monitorização.
