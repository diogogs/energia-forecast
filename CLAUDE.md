...
- [ ] **Módulo Open-Meteo** — previsões meteo (vento 100m, radiação, temp). Previous Runs (treino) + Forecast API (produção), modelo pinado `ecmwf_ifs025`. **Atenção ao budget Neon** (~169 MB de folga).
- [ ] **Camadas clean + features** — grelha horária, pivot das séries REN/EC, AsOfRepo (t_issue explícito).
