...
- [ ] **Deploy** — API em HF Spaces Docker (secret `DATABASE_URL_RO`, criar role read-only no Neon), dashboard em Streamlit Community Cloud (secret `API_BASE_URL`). Precisa das contas do utilizador.
- [ ] **Hardening final** — backup semanal `pg_dump`, `ops.dq_log`, drift, ENTSO-E cross-validation (opcional). Config DagsHub para MLflow remoto.
