...
- [ ] **Hardening final** — backup semanal `pg_dump`, `ops.dq_log`, drift, ENTSO-E cross-validation (opcional). Config DagsHub para MLflow remoto. Reativar auto-deploy do Render (ligar a GitHub App) e rodar o role `api_ro` (credencial esteve no chat).
