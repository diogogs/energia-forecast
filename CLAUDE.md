...
- [ ] **Iterar o dashboard** (prioridade do utilizador — é a cara do projeto): dar contexto, explicar o processo/rigor temporal, contar a história (não só gráficos). *A analisar em conjunto.*
- [ ] **Redeploy da API no Render** para publicar o endpoint `/monitoring/dq` (Manual Deploy → Deploy latest commit; ou religar a GitHub App para auto-deploy). O dashboard já tolera a ausência do endpoint entretanto.
