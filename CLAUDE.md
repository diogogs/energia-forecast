...
- [x] **Módulo REN — código completo, backfill AINDA NÃO corrido** (mesma branch). Contrato da API descoberto e verificado ao vivo (ADR-008): um só endpoint `ProductionBreakdown` dá `Consumption` (target Fase 1) + geração por tecnologia, 15-min. **Timezone Lisboa confirmado decisivamente** vs Energy-Charts PT (lag 0h, corr 1.0000, MW iguais ao decimal). `src/ingestion/sources/ren.py` (fetch+parse, âncora Europe/Lisbon DST-correta, skip de nulls, ticks .NET), `src/db/models.py` (`raw.ren_realised` tall + `meta.ren_series`), migração **0002 aplicada ao Neon** (tabela + índice + seed de 12 séries), `src/db/repositories/ren.py`, `src/ingestion/ren_backfill.py`. **35 testes verdes** (parser com fixtures DST 92/96/100, fetch mock, integração idempotência). Schema decidido por painel de design (3 propostas + juiz).

### A seguir (retomar aqui)
- [ ] **Correr o backfill REN** — `uv run python -m src.ingestion.ren_backfill --start 2024-01-01` (idempotente; `raw.ren_realised` está vazia). Depois: sanity check (cobertura, dias-DST, PT vs Energy-Charts).
- [ ] **Review adversarial** do diff REN (workflow) antes de merge da branch.
