...
- [ ] **⚠️ AÇÃO DO UTILIZADOR: redeploy da API no Render** (Manual Deploy → Deploy latest commit) — o dashboard novo precisa de `/history` e `/monitoring/dq`; tolera a ausência mas os painéis "today vs actual" e "ingestion log" ficam vazios até lá. Aproveitar para religar a GitHub App (auto-deploy).
- [ ] **Verificar o dashboard v2 live** no Streamlit Cloud (auto-redeploy com o push) + observar amanhã a 1ª emissão pontual (`late_issue=False` via retry tickets) a preencher o painel today-vs-actual.
- [ ] **Hardening restante** — drift, ENTSO-E cross-validation (opcional), config DagsHub para MLflow remoto, rodar o role `api_ro` (credencial esteve no chat).
