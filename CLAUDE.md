...
- [x] **`/ping` + auditoria de capacidade free-tier** ✅ (2026-07-10 noite). A auditoria para o projeto novo (dr-watch) revelou um problema NOSSO nascido nesse mesmo dia: o keepalive de 10 min ao `/health` (que toca na BD) impedia o Neon de fazer autosuspend → ~430+ CU-h/mês contra o limite de **100 CU-h/projeto/mês** do free plan → suspensão a meio do mês. Fix: endpoint **`/ping` sem BD** (Render fica quente, Neon dorme); workflow fallback atualizado; job do cron-job.org apontado ao /ping pelo utilizador. Outros números da auditoria: Neon energia a **440/512 MB (86%)** — alavanca ADR-009 no radar; Render 720/750h consumidas pela API 24/7 (sem espaço para 2º serviço — o dr-watch vai de Vercel serverless); cron-job.org sem limites relevantes. 125 testes verdes (+/ping, +ENTSO-E, +history).
...
- **Contexto paralelo:** site pessoal LIVE em [diogogs.github.io](https://diogogs.github.io) (`C:\dev\diogogs.github.io`, Astro) com o write-up publicado; projeto novo **dr-watch** em `C:\dev\dr-watch` (dia 1 completo). Mapa de tudo: `C:\dev\CLAUDE.md`.
